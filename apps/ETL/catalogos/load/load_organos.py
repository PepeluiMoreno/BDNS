...
def formar_id(tipo: TipoOrganoEnum, id_num: int) -> str:
    return f"{tipo.value}{id_num}"

def generar_id_unico(session, id_base):
    nuevo_id = id_base
    contador = 1
    while session.get(Organo, nuevo_id):
...
def insertar_organo(session, organo_dict):
    ...
    if existente:
        ...
        if conflicto:
            ...
            return insertar_organo(session, organo_dict)
        else:
...
def insertar_geografico(session, id_num, id_padre, nombre):
    id_ = formar_id( TipoOrganoEnum.GEOGRAFICO, id_num)
...
def procesar_estado(session):
    logger.info("Procesando órganos estatales...")
    resp = requests.get(BASE_URL + "C")
    resp.raise_for_status()
    ministerios = resp.json()
    id_padre_raiz = formar_id( TipoOrganoEnum.GEOGRAFICO, 0)

    for ministerio in ministerios:
        min_id = formar_id( TipoOrganoEnum.CENTRAL, ministerio['id'])
        ...
        for hijo in ministerio.get("children", []):
            hijo_id = formar_id( TipoOrganoEnum.CENTRAL, hijo['id'])
            hijo_nombre = hijo['descripcion']
            insertar_organo(session, {
                'id': hijo_id,
                'nombre': hijo_nombre,
...
def procesar_otros(session):
    ...
    id_padre_raiz = formar_id( TipoOrganoEnum.GEOGRAFICO, 0)
    for otro in otros:
        otro_id = formar_id( TipoOrganoEnum.OTRO, otro['id'])
        ...
        for hijo in otro.get("children", []):
            hijo_id = formar_id(TipoOrganoEnum.OTRO, hijo['id'])
            hijo_nombre = hijo['descripcion']
            insertar_organo(session, {
                'id': hijo_id,
                'nombre': hijo_nombre,
...
def procesar_locales(session):
    ...
    for provincia in provincias:
        ...
        id_prov_alfa = formar_id( TipoOrganoEnum.GEOGRAFICO, id_prov)
        ...
        for ccaa, lista_provs in MAPA_PROVINCIAS.items():
            ...
            if nombre_prov_norm in lista_norm:
                ccaa_nombre = ccaa
                ccaa_obj = None
                for posible in session.query(Organo).filter(Organo.tipo ==  TipoOrganoEnum.GEOGRAFICO):
        ...
        for municipio in provincia.get('children', []):
            ...
            id_muni_alfa = formar_id( TipoOrganoEnum.GEOGRAFICO, id_muni)
            insertar_organo(session, {
                'id': id_muni_alfa,
                'nombre': nombre_muni,
            ...
            for ayto in municipio.get('children', []):
                ...
                id_ayto = ayto['id']
                id_ayto_alfa = formar_id( TipoOrganoEnum.LOCAL, id_ayto)
                insertar_organo(session, {
                    'id': id_ayto_alfa,
                    'nombre': nombre_ayto,
...
def load_organos(session):
    logger.info("Inicio del poblamiento de órganos...")
    id_raiz_geo = formar_id(TipoOrganoEnum.GEOGRAFICO, 0)
    ...
    try:
        procesar_estado(session)
        session.commit()
        procesar_autonomicas(session)
        session.commit()
        procesar_locales(session)
        session.commit()
        procesar_otros(session)
        session.commit()
